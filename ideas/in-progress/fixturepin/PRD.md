...
Hash, label, and verify local fixture trees, recording schema hints and refresh notes so test suites can detect accidental fixture drift before misleading passes.

## Why now

Developers increasingly hand terminal work to coding agents, CI bots, and maintainers, but the handoff artifacts are often messy logs, screenshots, or private chat excerpts. FixturePin keeps the workflow local-first, deterministic, inspectable, and safe to share.
...
- Public GitHub repo under `rogerchappel/fixturepin` with useful description/topics.
