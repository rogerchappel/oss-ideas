...
A continual-learning context index for agents that gathers local traces, chats, and tool logs into searchable memory packs.
...
- `openamnesia` — source inspiration: https://github.com/vincentkoc/openamnesia (Python, stars/forks signal: 22).
...
- Ingest local transcript fixtures
- Normalize messages/tool calls/files
- Build searchable memory manifest
- Export citations and forgetting policy
...
```bash
memoryharbor --help
memoryharbor inspect ./fixtures/sample --output ./out
```
...
Build `memoryharbor` as a renamed, local-first OSS idea inspired by `openamnesia`. Preserve attribution, avoid direct copying, and focus V1 on deterministic fixtures, clear safety boundaries, and practical agent/developer workflow value.
