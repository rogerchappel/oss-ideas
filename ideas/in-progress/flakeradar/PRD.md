...
A tiny command harness that runs local checks repeatedly, compares outputs, and emits a fixture-backed flake report developers can trust.
...
Intermittent tests and CLIs are expensive in agent loops: a single lucky pass can hide nondeterminism. V1 focuses on local repeated command execution, stable diffing, and concise failure classification.
...
Any local shell command plus optional fixture directories.

## V1 CLI

```bash
flakeradar run -- npm test --repeat 5 --out flake-report.md
...
- GitHub repository is public under `rogerchappel/flakeradar` with useful description and topics.
