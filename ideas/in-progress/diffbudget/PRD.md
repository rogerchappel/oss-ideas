...
Score a git diff against configurable budgets for changed files, risky paths, generated output, dependency churn, and missing tests; emit human and JSON reports for agent gates.

## Why now

Developers increasingly hand terminal work to coding agents, CI bots, and maintainers, but the handoff artifacts are often messy logs, screenshots, or private chat excerpts. DiffBudget keeps the workflow local-first, deterministic, inspectable, and safe to share.
...
- Public GitHub repo under `rogerchappel/diffbudget` with useful description/topics.
