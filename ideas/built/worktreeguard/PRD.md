...
WorktreeGuard is a local-first TypeScript CLI that audits git worktree layouts for unsafe edits, dirty mains, duplicate branches, and publish readiness. It is built for developers and agentic coding loops that need deterministic evidence, tidy fixtures, and no surprise network calls. It is a guardrail for the exact failure mode this factory avoids: touching the wrong checkout.
...
- Agents and maintainers working across many isolated git worktrees
...
```bash
worktreeguard scan /path/to/workspace --out worktrees.md\nworktreeguard check --fail-on dirty-main,duplicate-branch
```
...
- GitHub repository is public under `rogerchappel/worktreeguard` with useful description and topics.
