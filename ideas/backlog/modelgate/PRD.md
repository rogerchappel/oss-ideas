...
A small local LLM gateway planner that audits provider configs, costs, and fallback rules without proxying secrets by default.
...
- `litellm fork` — source inspiration: https://github.com/vincentkoc/litellm (Python, stars/forks signal: not listed).
...
- Read redacted provider config fixtures
- Estimate routing/cost tables
- Validate missing fallback and budget rules
- Emit review report
...
```bash
modelgate --help
modelgate inspect ./fixtures/sample --output ./out
```
...
Build `modelgate` as a renamed, local-first OSS idea inspired by `litellm fork`. Preserve attribution, avoid direct copying, and focus V1 on deterministic fixtures, clear safety boundaries, and practical agent/developer workflow value.
