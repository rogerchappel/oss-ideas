...
An OpenClaw QQ channel adapter test kit with fixtures, message-shape validation, and safe local replay.
...
- `openclaw-qqbot fork` — source inspiration: https://github.com/vincentkoc/openclaw-qqbot (TypeScript, stars/forks signal: 1).
...
- Define QQ message fixtures
- Validate inbound/outbound payloads
- Replay local channel events
- Document auth and safety boundaries
...
```bash
qqbridge --help
qqbridge inspect ./fixtures/sample --output ./out
```
...
Build `qqbridge` as a renamed, local-first OSS idea inspired by `openclaw-qqbot fork`. Preserve attribution, avoid direct copying, and focus V1 on deterministic fixtures, clear safety boundaries, and practical agent/developer workflow value.
