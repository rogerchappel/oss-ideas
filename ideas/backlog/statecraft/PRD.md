...
A state-management recipe auditor that checks frontend repos for predictable slices, async flows, and test coverage.
...
- `redux-toolkit fork` — source inspiration: https://github.com/vincentkoc/redux-toolkit (TypeScript, stars/forks signal: not listed).
...
- Scan Redux-style project fixtures
- Detect missing tests and anti-patterns
- Generate migration checklist
- No app code mutation by default
...
```bash
statecraft --help
statecraft inspect ./fixtures/sample --output ./out
```
...
Build `statecraft` as a renamed, local-first OSS idea inspired by `redux-toolkit fork`. Preserve attribution, avoid direct copying, and focus V1 on deterministic fixtures, clear safety boundaries, and practical agent/developer workflow value.
